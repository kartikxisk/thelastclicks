...
>
    ...
    @php
        ...
        $featured = $service->featured_slug
            ? \App\Models\Portfolio::where('slug', $service->featured_slug)->first()
            : null;
        $featuredCover = $featured?->getFirstMediaUrl('cover') ?: $featured?->cover_url;
        $cta = $service->cta ?? [];
    @endphp
    ...
    {{-- 07 FEATURED CASE --}}
    @if ($featured)
        <section class="pp-case" data-screen-label="06 Featured case">
            <div class="wrap">
                <a class="pp-case__card spotlight" href="{{ url('/portfolio/'.$service->featured_slug) }}">
                    <div class="pp-case__media"><img src="{{ $featuredCover ?: $heroImg }}" alt="{{ $featured->title }}" loading="lazy" decoding="async"></div>
                    <div class="pp-case__body">
                        <span class="pp-case__eyebrow">Featured · {{ $service->title }}</span>
                        <h2 class="pp-case__title">{{ $featured->title }}</h2>
                        @if ($featured->location || $featured->year)
                            <p class="pp-case__meta">{{ collect([$featured->location, $featured->year])->filter()->implode(' · ') }}</p>
                        @endif
                        <span class="pp-case__cta">Read the case <span class="arr"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M5 19L19 5M19 5H8M19 5V16"/></svg></span></span>
                    </div>
                </a>
            </div>
        </section>
            ...
            <div class="wrap">
                <div class="services__head">
                    <div>
                        ...
                        <h2 class="section__title" data-split>Recent <em>projects.</em></h2>
                    </div>
                </div>
                <div class="work-grid">
